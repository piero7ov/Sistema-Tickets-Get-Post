<?php
// Listado (SELECT) + Nuevo (INSERT) + Cambiar estado (UPDATE) + Borrar (DELETE) + Ver ticket (GET vista=ver&id=)

$vista = isset($_GET["vista"]) ? (string)$_GET["vista"] : "list";
if ($vista !== "list" && $vista !== "new" && $vista !== "ver") $vista = "list";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // a dónde volvemos (para mantener filtros o volver a ver ticket)
...
if ($vista === "ver") {
  ...
  // volver (viene urlencoded desde el listado)
  if (isset($_GET["return"])) {
    $tmp = rawurldecode((string)$_GET["return"]);
    if ($tmp !== "" && strpos($tmp, "?") === 0) $returnFromView = $tmp;
  } else {
    $returnFromView = $returnHere; // si entraste desde el mismo listado
  }
  ...
  // URL de "esta vista" para quedarte aquí tras cambiar estado
      ...
      <?php else: ?>
        ...
        <div class="box">
          ...
          <div style="margin-top:8px;">
            <!-- Cambiar estado y quedarte en esta vista -->
