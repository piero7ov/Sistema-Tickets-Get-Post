<?php
// Listado (SELECT) + Nuevo (INSERT) + Cambiar estado (UPDATE) + Borrar (DELETE) + Ver (GET) + Editar (GET+POST UPDATE)
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($accion === "editar") {
    ...
    if ($id <= 0) {
      ...
      // Tras editar: ir a "ver ticket" y conservar return del listado
      $go = "?" . http_build_query([
        "vista" => "ver",
        "id" => $id,
        "return" => $return
...
if ($vista === "edit") {
  ...
  if ($idEdit <= 0) {
    ...
    // Si NO venimos de un POST con error, rellenamos $old con lo de la BD
    if ($editTicket && $error === "") {
...
<body>
  ...
  <main>
    ...
    <?php elseif ($vista === "ver"): ?>
      ...
      <h1>Ver ticket</h1>
      <p class="muted">
        <a href="<?= htmlspecialchars($returnFromView, ENT_QUOTES, "UTF-8") ?>">← Volver al listado</a>
        <?php if ($ticket): ?>
          · <a href="?vista=edit&id=<?= (int)$ticket["id"] ?>&return=<?= rawurlencode($returnFromView) ?>">Editar</a>
        <?php endif; ?>
      </p>

      <?php if ($verError !== ""): ?>
        <div class="error"><?= htmlspecialchars($verError, ENT_QUOTES, "UTF-8") ?></div>
      <?php else: ?>
        ...
        <div class="box">
          ...
          <div style="margin-top:8px;">
            <form class="inline" method="post" action="">
              ...
              <input type="hidden" name="return" value="<?= htmlspecialchars($selfView, ENT_QUOTES, "UTF-8") ?>">
            ...
            <form class="inline" method="post" action=""
              ...
              <input type="hidden" name="id" value="<?= (int)$ticket["id"] ?>">
              <input type="hidden" name="return" value="<?= htmlspecialchars($returnFromView, ENT_QUOTES, "UTF-8") ?>">
        ...
        <table>
          <tbody>
            ...
            <tr><th>Usuario</th><td><?= htmlspecialchars((string)$ticket["minecraft_user"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Tipo</th><td><?= htmlspecialchars((string)$ticket["tipo"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Prioridad</th><td><?= htmlspecialchars((string)$ticket["prioridad"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Estado</th><td><?= htmlspecialchars((string)$ticket["estado"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Creado</th><td><?= htmlspecialchars((string)$ticket["creado_en"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Actualizado</th><td><?= htmlspecialchars((string)$ticket["actualizado_en"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Título</th><td><?= htmlspecialchars((string)$ticket["titulo"], ENT_QUOTES, "UTF-8") ?></td></tr>
            <tr><th>Descripción</th><td><?= nl2br(htmlspecialchars((string)$ticket["descripcion"], ENT_QUOTES, "UTF-8")) ?></td></tr>
          </tbody>
        </table>
    ...
    <?php elseif ($vista === "edit"): ?>
      ...
      <p class="muted">
        <?php if ($editTicket): ?>
          <a href="?vista=ver&id=<?= (int)$editTicket["id"] ?>&return=<?= rawurlencode($returnFromEdit) ?>">← Volver a ver</a>
        <?php else: ?>
          <a href="<?= htmlspecialchars($returnFromEdit, ENT_QUOTES, "UTF-8") ?>">← Volver al listado</a>
        <?php endif; ?>
      </p>

      <?php if ($editError !== ""): ?>
        <div class="error"><?= htmlspecialchars($editError, ENT_QUOTES, "UTF-8") ?></div>
      <?php endif; ?>
      ...
      <?php if ($editTicket): ?>
        <div class="box">
          <form method="post" action="<?= htmlspecialchars($selfEdit, ENT_QUOTES, "UTF-8") ?>">
            <input type="hidden" name="accion" value="editar">
            <input type="hidden" name="id" value="<?= (int)$editTicket["id"] ?>">
            <input type="hidden" name="return" value="<?= htmlspecialchars($returnFromEdit, ENT_QUOTES, "UTF-8") ?>">
            ...
            <div style="margin-top:10px;">
              <div class="muted">Descripción</div>
              <textarea name="descripcion"><?= htmlspecialchars($old["descripcion"], ENT_QUOTES, "UTF-8") ?></textarea>
            </div>

            <div style="margin-top:10px;">
              <button type="submit">Guardar cambios</button>
              <a href="?vista=ver&id=<?= (int)$editTicket["id"] ?>&return=<?= rawurlencode($returnFromEdit) ?>">Cancelar</a>
            </div>
          </form>
        </div>
      <?php endif; ?>
          ...
          <?php else: ?>
            <?php foreach ($tickets as $t): ?>
              <tr>
                ...
                <td>
                  <a href="?vista=ver&id=<?= (int)$t["id"] ?>&return=<?= rawurlencode($returnHere) ?>">
                ...
                <td>
                  <a href="?vista=edit&id=<?= (int)$t["id"] ?>&return=<?= rawurlencode($returnHere) ?>">Editar</a>
