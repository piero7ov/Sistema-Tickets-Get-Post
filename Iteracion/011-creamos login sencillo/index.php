...
/* =========================================================
   ...
   - Si no hay sesión, redirige a login.php y vuelve aquí.
   ========================================================= */
if (empty($_SESSION["admin"])) {
  $return = $_SERVER["REQUEST_URI"] ?? "index.php?vista=list";
...
/* Evita redirects externos (solo rutas locales) */
function safeReturn($r, $fallback){
  $r = (string)$r;
  if ($r === "") return $fallback;

  // Bloquear protocolos tipo http:, https:, javascript:, etc.
  if (preg_match('~^[a-z][a-z0-9+.-]*:~i', $r)) return $fallback;

  // Bloquear URLs tipo //dominio.com
  if (strpos($r, "//") === 0) return $fallback;

  return $r; // permite ?, /ruta, index.php?...
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $return = safeReturn($_POST["return"] ?? "", "?vista=list");
...
if ($vista === "ver") {
  ...
  if (isset($_GET["return"])) {
    $returnFromView = safeReturn($_GET["return"], $returnHere);
  } else {
...
if ($vista === "edit") {
  ...
  if (isset($_GET["return"])) {
    $returnFromEdit = safeReturn($_GET["return"], $returnHere);
  } else {
...
<body>
  ...
  <aside class="sidebar">
    ...
    <div class="foot">Minecraft · <?= h($_SESSION["admin_user"] ?? "admin") ?></div>
  </aside>
